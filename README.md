...
Compile the DDSSE impementation

```bash
make genesse
```
Run the DDSSE impementation
...
DRangeSSE.Update(λ, Index): Encrypts the index using the GeneSSE_Update function to produce the encrypted database (EDB).

###Search:
GenRangeCover(Tree, [x, y]): Generates the minimal covering set of nodes that cover the query range [x, y].
DRangeSSE.Search(query, EDB): Executes the disjunctive search query on the encrypted database by calling GeneSSE_Search function and returns the results.
